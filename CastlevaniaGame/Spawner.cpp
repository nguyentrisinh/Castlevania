...
Spawner::Spawner(LPD3DXSPRITE _SpriteHandler, World *_manager) :Zone::Zone(_SpriteHandler, _manager)
{
	manager = _manager;
	...
	int rubbish1, int rubbish2, int rubbish3)
{
	...
	for (int i = 0; i < numberEnemy; i++)
	{
		timers[i] = TIME_RESPAWN * (0.1f) + ((float)i * 0.75f);
	}
